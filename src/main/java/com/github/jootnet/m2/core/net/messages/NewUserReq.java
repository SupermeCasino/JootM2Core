...
public class NewUserReq extends Message {
	...
	static {
		deSerializers.put(MessageType.NEW_USER_REQ, buffer -> {
			var una = unpackString(buffer);
			var psw = unpackString(buffer);
			var name = unpackString(buffer);
			var q1 = unpackString(buffer);
			var a1 = unpackString(buffer);
			var q2 = unpackString(buffer);
			var a2 = unpackString(buffer);
			var tel = unpackString(buffer);
			var iPhone = unpackString(buffer);
			var mail = unpackString(buffer);
